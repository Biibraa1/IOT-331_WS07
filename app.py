...
app = Flask(__name__)
server_ip = "http://192.168.251.163:4000"
@app.route("/")
def home():
...
def thingspeak():
    ...
    url = "https://thingspeak.com/channels/1122157/feed.json"
...
def nodemcu():
    headers = {'Accept': 'application/json'}
    r = requests.get(server_ip+'/Sensor?_sort=id&_order=desc&_limit=1', headers=headers)
    return r.json();
